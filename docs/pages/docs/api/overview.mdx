...
For a more complete overview of the publishing process, please navigate to the dedicated [Publish documentation](/docs/guides/publish).

## Editing
{/* TODO */}

For a more detailed overview of all configurations provided for the edit API, head over to the [Edit documentation](/docs/guides/edit).
...
// ---cut---
// make sure we have a single job reference
const job = Array.isArray(computeJob) ? computeJob[0] : computeJob

const { jobId } = job // [!code focus]
```
...
```
For a deep-dive into everything possible with the compute APIs, head over to the [Compute-to-Data documentation](/docs/guides/compute).
