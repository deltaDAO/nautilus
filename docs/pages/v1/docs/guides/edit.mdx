...
For a detailed look at the builder class, please refer to the [`AssetBuilder` documentation](/v1/docs/api/AssetBuilder).
...
const result = await nautilus.edit(asset)
```
Refer to the [`addCredentialAddresses()`](/v1/docs/api/assetbuilder/addCredentialAddresses) and [`removeCredentialAddresses`](/v1/docs/api/assetbuilder/removeCredentialAddresses) api documentations for more information.
...
To learn more about all available lifecycle states, refer to the [`nautilus.setAssetLifecycleState()` API documentation](/v1/docs/api/nautilus/setAssetLifecycleState).
...
```
Refer to the [`nautilus.setServicePrice()` API documentation](/v1/docs/api/nautilus/setServicePrice) for further information.
...
You use the [`ConsumerParameterBuilder`](/v1/docs/api/ConsumerParameterBuilder) to build each parameter and attach it to a service via `addConsumerParameter`.
